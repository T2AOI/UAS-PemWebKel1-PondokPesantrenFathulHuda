...
WHERE tb_mengajar.id_mengajar='$_GET[pelajaran]' AND tb_mengajar.id_mkelas='$_GET[kelas]'  AND tb_thajaran.status=1 AND tb_semester.status=1  ");
	...
	foreach ($kelasMengajar as $d)
		...
		$qry = mysqli_query($con, "SELECT _logabsensi.id_siswa, COUNT(*) AS total_kehadiran
		FROM _logabsensi
 	...
 	<tr>
 		...
 		<td>
 			<center>
 				...
 				<h1>
 					ABSENSI SISWA <br>
 					<small>PONDOK PESANTREN FATHUL HUDA</small>
 				</h1>
 				<hr>
 				<em>
 					Jl. Raya Kediri-Blitar RT04/RW04, Ds. Ringinanom, Kec. Udanawu, Kab. Blitar
 					<br>No. Telepon : 085856363595
 				</em>
 ...
 <table width="100%" border="1" cellpadding="2" style="border-collapse: collapse;">
 	...
 	<?php
		...
		$siswaData = mysqli_query($con, "SELECT nama_siswa, jk FROM tb_siswa");
		$no = 1;
		foreach ($qry as $ds) {
			$siswa = mysqli_fetch_assoc($siswaData);
		...
		?>
 		<tr bgcolor="<?= $warna; ?>">
 			...
 			<?php
				// Fetch attendance data for each student and pertemuan
				for ($i = 1; $i <= $tglTerakhir; $i++) {
					// Kolom absensi untuk setiap pertemuan
					echo "<td align='center' bgcolor='white'>";
					$ket = mysqli_query($con, "SELECT MAX(_logabsensi.id_presensi) AS id_presensi, _logabsensi.id_siswa, _logabsensi.pertemuan_ke, MAX(nama_siswa) AS nama_siswa, MAX(ket) AS ket FROM _logabsensi
                                            INNER JOIN tb_mengajar ON _logabsensi.id_mengajar=tb_mengajar.id_mengajar
                                            INNER JOIN tb_semester ON tb_mengajar.id_semester=tb_semester.id_semester
                                            INNER JOIN tb_thajaran ON tb_mengajar.id_thajaran=tb_thajaran.id_thajaran
                                            INNER JOIN tb_siswa ON _logabsensi.id_siswa=tb_siswa.id_siswa
                                            WHERE _logabsensi.pertemuan_ke='$i' AND _logabsensi.id_siswa='$ds[id_siswa]' AND _logabsensi.id_mengajar='$_GET[pelajaran]' AND tb_mengajar.id_mkelas='$_GET[kelas]' AND tb_thajaran.status=1 AND tb_semester.status=1 GROUP BY _logabsensi.id_siswa, _logabsensi.pertemuan_ke");
